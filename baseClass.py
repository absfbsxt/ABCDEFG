...
class baseParent:
    ...
    def __init__(self, bottom_left, top_right, name):
        ...
        self.name = name
        self.rotation = 0
        # redArea means to points those were occupied by furnitures, also a library of all furniture points [{id,point,furniture},{~},{~},...]
        self.redAreas = []
...
#         print(point.x + furniture.top_right.x , self.top_right.x , point.y+furniture.top_right.y , self.top_right.y)
    ...
    def center(self, i):
        ...
        """
        temp1 = self.redAreas[i]["point"].x + self.redAreas[i]["furniture"].top_right.x/2
        temp2 = self.redAreas[i]["point"].y + self.redAreas[i]["furniture"].top_right.y/2
...
class Room(baseParent):
    ...
    def move(self, itemx, direction):
        ...
        Parameters:
        itemx(Furniture) = Furniture object that is to be moved
        direction(int): 1,2,3,4 for up down left right direction
        index(int) = Index of furniture in room objects list
...
class Furniture():
    ...
    def calcTransitionProbability(self):
        """Description:
        mark: cost function for single furniture object
        """

class Group(baseParent):
    """Description:
    ...
    def Add(self, furniture):
        ...
        Parameter:
        furniture(Furniture) furnitures waiting to be added into group
    ...
    # def initial_arrangement(self):
    #     """
    #     Description:
    #     generate initial furniture arrangement for one group
    ...
    def group_points(self):
        """
        Description:function using to calculate group boundary
        """
    ...
    def calcCenter(self):
        """Description:
            Calculate barycenter of a group
        
        Returns :
        Barycenter point of a group

        """

        temp_x, temp_y = 0, 0
        for idx in enumerate(self.redAreas):
            temp_x += self.redAreas[idx]["point"].x
            temp_y += self.redAreas[idx]["point"].y

        barycenter_x = temp_x/len(self.redAreas)
        barycenter_y = temp_y/len(self.redAreas)

        return Point(barycenter_x, barycenter_y)
        
    def move(self, distance):
        """
        Description:
        move every furniture in group in a random rate
        
        Parameter:
        distance: sigma of gaussian distribution
        ...
        for idx in self.redAreas:
            # line 1013
            # distance generated by norm distribution
    ...
    def Metropolis(self, old_c, new_c):
        """
        Description:
        ...
        if new_c < old_c:
            ...
            probability = math.exp(self.beta*(old_c - new_c))
